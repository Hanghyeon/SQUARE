...
public class UserArmControllerScript : MonoBehaviour {
	...
	void Update () {
		...
		if ((Input.GetMouseButton(0) ||/* (GameObject.FindObjectOfType<ShotSender>() != null) ?*/ ShotSender.Singleton.isFire /*: (Input.GetMouseButton(0))*/) && !ShootSettings.automaticFire
        ...
        if ((Input.GetMouseButton(0) || ShotSender.Singleton.isFire) && ShootSettings.automaticFire == true
		    //Disable shooting while running and jumping
			&& !isReloading && !outOfAmmo && !isShooting && !isAimShooting && !isRunning && !isJumping) {
			//Shoot automatic
		...
		if ((Input.GetMouseButton(0) || ShotSender.Singleton.isFire) && ShootSettings.grenade == true && 
