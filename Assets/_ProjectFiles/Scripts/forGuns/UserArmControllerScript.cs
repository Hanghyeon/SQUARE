...
public class UserArmControllerScript : MonoBehaviour {

    public bool nowReloading { get { return isReloading; } }
	...
	void Awake () {
        ...
        GameObject go = GameObject.Find("ShotImageTarget/ShotSender");
        if (go != null)
            sSender = go.GetComponent<ShotSender>();
		...
		//Hide muzzleflash and light at start, disable for projectile, grenade and melee weapons
	...
	void Update () {
		...
		if ((Input.GetMouseButton(0) ||/* (GameObject.FindObjectOfType<ShotSender>() != null) ?*/ sSender.isFire /*: (Input.GetMouseButton(0))*/) && !ShootSettings.automaticFire
			...
			if (!ShootSettings.projectileWeapon && !ShootSettings.useShotgunSpread && !MeleeSettings.isMeleeWeapon && !ShootSettings.grenade) {
				Shoot();
        ...
        if ((Input.GetMouseButton(0) || sSender.isFire) && ShootSettings.automaticFire == true
		    //Disable shooting while running and jumping
			&& !isReloading && !outOfAmmo && !isShooting && !isAimShooting && !isRunning && !isJumping) {
			//Shoot automatic
		...
		if ((Input.GetMouseButton(0) || sSender.isFire) && ShootSettings.grenade == true && 
        ...
        if ((Input.GetKeyDown(KeyCode.R) || ReloadSender.Singleton.isReload)
                && !isReloading 
                && !ShootSettings.projectileWeapon
				&& !MeleeSettings.isMeleeWeapon && !ShootSettings.grenade){
			Reload ();
		}
	...
	void Shoot() {
		...
		if (Physics.Raycast (Spawnpoints.bulletSpawnPoint.transform.position, 
            ...
            if (hit.rigidbody != null)
            {
                hit.rigidbody.AddTorque(ray.direction * ShootSettings.bulletForce, ForceMode.Impulse);
                hit.rigidbody.AddForce(ray.direction * ShootSettings.bulletForce, ForceMode.Impulse);
            }
			...
			if (hit.transform.tag == "Target") {
                ...
                //Check is User Custom Component?
                if (hit.transform.gameObject.GetComponent<UserTargetScript>())
                {
                    //Toggle the isHit bool on the target object
                    hit.transform.gameObject.GetComponent<UserTargetScript>().isHit = true;
                }
                else
                {
                    //Toggle the isHit bool on the target object
